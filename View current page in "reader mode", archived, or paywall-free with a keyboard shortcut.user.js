...
(function () {
    ...
    function handleEvent(e) {
        // this function is called when a keys are pressed on the keyboard
        if (e.target.getAttribute("contenteditable") == "true") {
            // ignore shortcuts when a document is being edited (like Google Docs or Wikipedia for example)
            return;
        }
        for (const shortcut of SHORTCUT_CONFIGS) {
            // go over each configured shortcut
            if (!shortcut.letter) {
                // validate that there is a letter
                console.error("No shortcut letter configured!");
            } else if (shortcut.letter.length != 1) {
                // and that there is only *one* letter per shortcut
                console.error(
                    `Invalid shortcut letter configured: '${shortcut.letter}' – expected a single letter, found ${shortcut.letter.length} instead`,
                );
            } else {
                // then check if the keyboard event matched a particular configured shortcut
                ...
                if (
                    ...
                    e.metaKey === isEnabled(shortcut, "needsModifierMeta")
                ) {
                    // matched!

                    const newLocation =
                        prefix + (isEnabled(shortcut, "encodeURI") ? encodeURI(location.href) : location.href);
